...
class RLlibStarCraft2Env(rllib.MultiAgentEnv):
    ...
    def __init__(self, **smac_args):
        ...
        self._ready_agents = []
        # Whether to use custom model (masked actions)
    ...
    def step(self, action_dict):
        ...
        # If dont use mask for actions:
        ## 1. Get available actions
        if not self.masked_actions:
            avail_agent = self._env.get_avail_actions()
            actions_mask = np.ones(len(avail_agent))
        for i in self._ready_agents:
            ...
            # 2. set 0 for not available actions
            else:
                if avail_agent[i][action_dict[i]] == 1:
                    actions.append(action_dict[i])
                else:
                    actions_mask[i] = 0
                    actions.append(avail_agent[i].index(1))
        ...
        self._ready_agents = list(range(len(obs_list)))
        return return_obs, rew, done, info
